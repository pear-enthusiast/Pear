...
Pear.Ascii = "qwertyuiopasdfghjklzxcvbnmQWRTYUIOPASDFGHJKLZXCVBNM";
Pear.GLOBAL_ENVIRONMENT = {};
Pear.Windows = {};
Pear.FontSemiBold = Font.new('rbxasset://fonts/families/GothamSSm.json',Enum.FontWeight.SemiBold,Enum.FontStyle.Normal);
Pear.Flags = {};
Pear.Colors = {
	Black = Color3.fromRGB(16, 16, 16),
	Main = Color3.fromRGB(166, 211, 136),
	Slider = Color3.fromRGB(122, 168, 96)
};
Pear.DragBlacklist = {};
Pear.Version = '0.0.0';
Pear.Lucide = {
...
function Pear:IsMobile() : boolean
	return UserInputService.TouchEnabled;	
end;

function Pear:RandomString() : string
...
function Pear:GetTextSize(Text : TextLabel,CustomFont: Enum.Font) : Vector2
...
function Pear:IsMouseOverFrame(Frame : Frame) : boolean
...
function Pear:GetCalculatePosition(planePos: number, planeNormal: Vector3, rayOrigin: number, rayDirection: Vector3) : number
...
function Pear:CreateHover(Element : Frame,Callback : (boolean) -> any)
...
function Pear:GetIcon(name : string) : string
	return Pear.Lucide['lucide-'..tostring(name)] or Pear.Lucide[name] or Pear.Lucide[tostring(name)] or "";
end;

function Pear:SetIcon(name : string, value : string)
	Pear.Lucide[name] = value;
end;

function Pear:Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Pear:CreateAnimation(Instance: Instance , Time: number , Style : Enum.EasingStyle , Property : {[string] : any}) : Tween
...
function Pear:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		Pear:CreateAnimation(MoveFrame,Speed,nil,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Pear.DragBlacklist <= 0 then 
			dragToggle = true
...
function Pear:ScrollSignal(Scroll: ScrollingFrame,UIListLayout: UIListLayout,Type:string)
	if Type == 'X' then
...
function Pear:CreateResponse(args: {[string] : (any) -> any})
	local main = {};
...
function Pear:GetWindowFromElement(Element: GuiObject)
	for i,v in next , Pear.Windows do
...
function Pear:CreateOption(OptionButton: ImageButton): Elements
	Pear:CreateHover(OptionButton,function(bool)
		if bool then
			Pear:CreateAnimation(OptionButton,0.5,{
	...
	local OwnWindow = Pear:GetWindowFromElement(OptionButton);
	...
	ExtElementFrame.Name = Pear:RandomString()
	...
	DropShadow.Name = Pear:RandomString()
	...
	SpaceBox.Name = Pear:RandomString()
	...
	local ToggleExt = function(bool)
		if bool then
			...
			Pear:CreateAnimation(ExtElementFrame,0.45,{
				Size = UDim2.new(0, 200, 0, size)
			})

			Pear:CreateAnimation(DropShadow,0.45,{
				ImageTransparency = 0.750
			})

			Pear:CreateAnimation(UIStroke,0.45,{
				Transparency = 0
			})
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait(0.1)
					...
					Pear:CreateAnimation(ExtElementFrame,0.35,{
						Position = ud
					});
				end;
			end)
	...
	UserInputService.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
			if not Pear:IsMouseOverFrame(ExtElementFrame) and not Pear.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER then
				ToggleExt(false);
			end;
		end;
	end);

	local UIElements: Elements = Pear:CreateElements(ScrollingFrame,ScrollingFrame.ZIndex,Bindable);

	return UIElements;
end;

function Pear:CreateColorPicker(ColorBox: Frame,Transparency, Callback)
	Transparency = Transparency or 0;
	Callback = Callback or function() end;

	local OwnWindow = Pear:GetWindowFromElement(ColorBox);
	...
	local VisibleToggle = function(value)
		if value then
			...
			Pear:CreateAnimation(ColorPickBox,0.45,{
				ImageTransparency = 0,
				BackgroundTransparency = 0
			});

			Pear:CreateAnimation(MouseMovement,0.45,{
				ImageTransparency = 0,
			});

			Pear:CreateAnimation(UIStroke_2,0.45,{
				Transparency = 0
			});

			Pear:CreateAnimation(ColorRedGreenBlue,0.45,{
				BackgroundTransparency = 0
			});

			Pear:CreateAnimation(ColorRGBSlide,0.45,{
				BackgroundTransparency = 0
			});

			Pear:CreateAnimation(UIStroke_3,0.45,{
				Transparency = 0.75
			});

			Pear:CreateAnimation(ColorOpc,0.45,{
				BackgroundTransparency = 0
			});

			Pear:CreateAnimation(ColorOptSlide,0.45,{
				BackgroundTransparency = 0
			});

			Pear:CreateAnimation(UIStroke_4,0.45,{
				Transparency = 0.75
			});

			Pear:CreateAnimation(UIStroke_5,0.45,{
				Transparency = 0
			});

			Pear:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.45
			});

			Pear:CreateAnimation(CopyButton,0.45,{
				ImageTransparency = 0.45
			});

			Pear:CreateAnimation(hexCode,0.45,{
				BackgroundTransparency = 0.4
			});

			Pear:CreateAnimation(HexCodeText,0.45,{
	...
	ColorPickerFrame.Name = Pear:RandomString()
	...
	Pear:AddDragBlacklist(ColorPickerFrame);
	...
	ColorPickBox.Name = Pear:RandomString()
	...
	MouseMovement.Name = Pear:RandomString()
	...
	ColorRedGreenBlue.Name = Pear:RandomString()
	...
	ColorRGBSlide.Name = Pear:RandomString()
	...
	ColorOpc.Name = Pear:RandomString()
	...
	ColorOptSlide.Name = Pear:RandomString()
	...
	ColorOpt.Name = Pear:RandomString()
	...
	PasteButton.Name = Pear:RandomString()
	...
	CopyButton.Name = Pear:RandomString()
	...
	hexCode.Name = Pear:RandomString()
	hexCode.Parent = ColorOpt
	hexCode.BackgroundColor3 = Pear.Colors.Black
	...
	HexCodeText.Name = Pear:RandomString()
	...
	HexCodeText.FontFace = Pear.FontSemiBold
	...
	Pear:CreateHover(CopyButton,function(bool)
		if bool then
			Pear:CreateAnimation(CopyButton,0.45,{
	...
	Pear:CreateHover(PasteButton,function(bool)
		if bool then
			Pear:CreateAnimation(PasteButton,0.45,{
				ImageTransparency = 0.1
			})
	...
	PasteButton.MouseButton1Click:Connect(function()
		if Pear.GLOBAL_ENVIRONMENT.COLOR_COPY then
			local h,s,v = Pear.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHSV();

			Transparency = Pear.GLOBAL_ENVIRONMENT.COLOR_COPY.OPC;

			OldCode = h;
			CodeH = s;
			CodeV = v;

			HexCodeText.Text = "#" .. tostring(Pear.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB:ToHex());
			...
			ColorOpc.BackgroundColor3 = Pear.GLOBAL_ENVIRONMENT.COLOR_COPY.RGB;
	...
	CopyButton.MouseButton1Click:Connect(function()
		Pear.GLOBAL_ENVIRONMENT.COLOR_COPY = {
	...
	Pear:NewInput(ColorBox,function()
		VisibleToggle(true);
	end);
	...
	do
		...
		ColorPickerFrame.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				Pear.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = true;
		...
		ColorPickerFrame.InputEnded:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				IsPressM1 = false;
				Pear.GLOBAL_ENVIRONMENT.IS_HOLD_COLOR_PICKER = false;
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if not Pear:IsMouseOverFrame(ColorPickerFrame) then
					VisibleToggle(false);
				end;
			end;
		end)
		...
		ColorRedGreenBlue.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait()
					...
					Pear:CreateAnimation(ColorBox,0.5,{
						BackgroundColor3 = Color
					});

					Pear:CreateAnimation(ColorOpc,0.35,{
						BackgroundColor3 = Color
					});

					Pear:CreateAnimation(ColorPickBox,0.5,{
		...
		ColorPickBox.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				...
				while (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or IsPressM1) do task.wait();
					...
					Pear:CreateAnimation(MouseMovement,0.2,nil,{
						Position = UDim2.new(CodeH, 0, 1 - CodeV, 0)
					})
				end
			end
...
function Pear:RandomStr(len: number): string
	local main = "";

	for i = 1 , len do
		local max = #Pear.Ascii;
		local rand = math.random(1,max);

		main = main .. Pear.Ascii:sub(rand,rand);
	end;

	return main;
end;

function Pear:AddDragBlacklist(Frame: Frame)
	Frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			local finder = table.find(Pear.DragBlacklist , Frame);

			if not finder then
				table.insert(Pear.DragBlacklist , Frame);
			end;
		end;
	end);
	...
	Frame.InputEnded:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.MouseMovement then
			...
			if finder then
				table.remove(Pear.DragBlacklist , finder);
			end;
		end;
	end);
end;

function Pear:ProtectText(Label: TextLabel,Text: string)
	Label.RichText = true;

	local MainText = string.gsub(Text,'.',function(t)
		if not string.find(t,"[<>()\"']") then
			return string.format('<font %s="%s">%s</font>',Pear:RandomStr(5),Pear:RandomStr(5),t)
		end;
		return t;
	end);
...
function Pear:CreateDropdown(Parent: Frame, Default: string | {[string]: boolean}, Multiplier: boolean, AutoUpdate: boolean,Callback: (data: any) -> any)
	local Window = Pear:GetWindowFromElement(Parent);
	local Data = {};
	...
	Pear:AddDragBlacklist(DropdownItemFrame);
		...
		if value then
			...
			SPAWN_THREAD = task.spawn(function()
				while true do task.wait()
					...
					Pear:CreateAnimation(DropdownItemFrame,0.35,{
						Size = UDim2.new(0, 175, 0, math.clamp(baseSize,0,200))
					})
				end;
			end)

			Pear:CreateAnimation(UIStroke,0.35,{
				Transparency = 0
			})
	...
	DropdownItemFrame.Name = Pear:RandomString()
	...
	local new_button = function()
		...
		db_selected.Name = Pear:RandomString()
		...
		db_selected.FontFace = Pear.FontSemiBold
		db_selected.TextColor3 = Pear.Colors.Main
			...
			for i,v in next , Data do
				...
				if Multiplier then
					if (typeof(Default) == 'table' and (Default[v] or table.find(Default,v))) or Default == v then
						Selected[v] = true
						bth.TextColor3 = Pear.Colors.Main;
					else
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Pear:IsMouseOverFrame(DropdownItemFrame) then
				func(false);
				Toggle(false);
			end;
		end;
...
function Pear:CreateElements(Parent : Frame , ZIndex : number , Event : BindableEvent,SearchAPI: {Path: string,Memory : (name:string) -> any}) : Elements
	local elements = {};
	local FatalWindow = Pear:GetWindowFromElement(Parent);
	...
	function elements:AddToggle(Config: Toggle)
		...
		Pear:AddDragBlacklist(Toggle);

		Toggle_Name.Name = Pear:RandomString()
		...
		Toggle_Name.FontFace = Pear.FontSemiBold;
		...
		Pear:ProtectText(Toggle_Name,Config.Name);

		ValueFrame.Name = Pear:RandomString()
		ValueFrame.Parent = Toggle
		ValueFrame.AnchorPoint = Vector2.new(1, 0.5)
		ValueFrame.BackgroundColor3 = Pear.Colors.Black
		...
		ValueIcon.Name = Pear:RandomString()
		...
		OptionButton.Name = Pear:RandomString()
		...
		local toggleImg = function(value)
			if value then
				Pear:CreateAnimation(ValueIcon,0.45,{
					ImageTransparency = 0,
					ImageColor3 = Pear.Colors.Main,
		...
		local OpcToggle = function(value)
			if value then
				...
				Pear:CreateAnimation(OptionButton,0.45,{
					ImageTransparency = (Config.Option and 0.600) or 1
				})

				toggleImg(Config.Default)

				Pear:CreateAnimation(ValueFrame,0.45,{
					BackgroundTransparency = 0,
				})

				Pear:CreateAnimation(Toggle_Name,0.45,{
		...
		Pear:CreateHover(ValueFrame,function(b)
		...
		local Respons = Pear:CreateResponse({
			Rename = function(new_name)
				Toggle_Name.Text = new_name;
				Pear:ProtectText(Toggle_Name,new_name);
			end,
			GetValue = function()
			...
			Option = (Config.Option and Pear:CreateOption(OptionButton)) or nil;
		});

		if Config.Flag then
			Pear.WindowFlags[FatalWindow][Config.Flag.."Toggle"] = Respons;
		end;
	...
	function elements:AddSlider(Config: Slider)
		...
		Pear:AddDragBlacklist(Slider);

		Slider_Name.Name = Pear:RandomString()
		...
		Slider_Name.FontFace = Pear.FontSemiBold
		...
		boxli.Name = Pear:RandomString()
		boxli.Parent = ValueFrame
		boxli.BackgroundColor3 = Pear.Colors.Slider
		...
		ValueText.Name = Pear:RandomString()
		...
		local OpcToggle = function(value)
			if value then
				Pear:CreateAnimation(boxli,0.45,{
					BackgroundTransparency = 0,
				})
				...
				Pear:CreateAnimation(Slider_Name,0.45,{
					TextTransparency = 0.200
				})

				Pear:CreateAnimation(ValueText,0.45,{
		...
		local function update(Input)
			...
			local Value = Pear:Rounding(Main,Config.Round);
			...
			ValueFrame.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Pear:IsMouseOverFrame(ValueFrame) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		if Config.Flag then
			Pear.WindowFlags[FatalWindow][Config.Flag.."Slider"] = Respons;
		end;
	...
	function elements:AddButton(Config: Button)
		...
		Button.Name = Pear:RandomString()
		Button.Parent = Parent
		Button.BackgroundColor3 = Pear.Colors.Black
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		Button.Size = UDim2.new(1, -25, 0, 25)
		Button.ZIndex = ZIndex + 1
		Pear:AddDragBlacklist(Button);

		Button_Name.Name = Pear:RandomString()
		...
		Button_Name.FontFace = Pear.FontSemiBold
		...
		Pear:ProtectText(Button_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Pear:CreateAnimation(Button_Name,0.45,{
					TextTransparency = 0.4,
				})

				Pear:CreateAnimation(Button,0.45,{
					BackgroundTransparency = 0,
				})
			else
				Pear:CreateAnimation(Button_Name,0.45,{
		...
		Pear:CreateHover(Button,function(value)
			if value then
				Pear:CreateAnimation(Button,0.45,{
				...
				Pear:CreateAnimation(Button,0.45,{
					BackgroundColor3 = Pear.Colors.Black
				})
			end;
		end)

		Pear:NewInput(Button,function()
			Config.Callback();
		end)
		...
		return Pear:CreateResponse({
			Rename = function(new_name)
				...
				Pear:ProtectText(Button_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
	...
	function elements:AddColorPicker(Config: ColorPicker)
		...
		ColorPicker.Name = Pear:RandomString()
		...
		Pear:AddDragBlacklist(ColorPicker);

		ColorPicker_Name.Name = Pear:RandomString()
		...
		ColorPicker_Name.FontFace = Pear.FontSemiBold
		...
		local res = Pear:CreateColorPicker(ValueFrame,Config.Transparency,function(rgb,opc)
		...
		local OpcToggle = function(value)
			if value then
				Pear:CreateAnimation(ColorPicker_Name,0.45,{
		...
		if Config.Flag then
			Pear.WindowFlags[FatalWindow][Config.Flag.."ColorPicker"] = Respons;
		end;
	...
	function elements:AddDropdown(Config: Dropdown)
		...
		Dropdown.Name = Pear:RandomString()
		...
		Pear:AddDragBlacklist(Dropdown);

		Dropdown_Name.Name = Pear:RandomString()
		...
		Dropdown_Name.FontFace = Pear.FontSemiBold
		...
		Pear:ProtectText(Dropdown_Name,Config.Name);
		...
		Value_Text.Name = Pear:RandomString()
		...
		Value_Text.FontFace = Pear.FontSemiBold
		...
		local res;
		res = Pear:CreateDropdown(ValueFrame,Config.Default,Config.Multi,Config.AutoUpdate,function(args)
			Config.Default = args;
			Value_Text.Text = DataParser(Config.Default);
		...
		res:on_toggle(function(b)
			if b then
				Pear:CreateAnimation(icon,0.35,{
					Rotation = -180,
					ImageColor3 = Pear.Colors.Main
				})
			else
		...
		local OpcToggle = function(value)
			if value then
				Pear:CreateAnimation(Dropdown_Name,0.45,{
					TextTransparency = 0.2,
				})
				...
				Pear:CreateAnimation(icon,0.45,{
					ImageTransparency = 0
				})

				Pear:CreateAnimation(Value_Text,0.45,{
		...
		local Respons = Pear:CreateResponse({
			Rename = function(new_name)
				...
				Pear:ProtectText(Dropdown_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			Pear.WindowFlags[FatalWindow][Config.Flag.."Dropdown"] = Respons;
		end;
	...
	function elements:AddKeybind(Config: Keybind)
		...
		Keybind.Name = Pear:RandomString()
		...
		Pear:AddDragBlacklist(Keybind);

		Keybind_Name.Name = Pear:RandomString()
		...
		Keybind_Name.FontFace = Pear.FontSemiBold
		...
		Pear:ProtectText(Keybind_Name,Config.Name);
		...
		local OpcToggle = function(value)
			if value then
				Pear:CreateAnimation(Keybind_Name,0.45,{
					TextTransparency = 0.2,
				})
		...
		local Respons = Pear:CreateResponse({
			Rename = function(new_name)
				...
				Pear:ProtectText(Keybind_Name,new_name);
			end,
			GetValue = function()
				return Config.Default;
			end,
		...
		if Config.Flag then
			...
			Pear.WindowFlags[FatalWindow][Config.Flag.."Keybind"] = Respons;
		end;
...
function Pear:CreateConfigWindow(Root: ScreenGui , Fatal , Button: ImageButton)
	...
	Pear:AddDragBlacklist(ConfigWindowFrame);
	...
	local ElementToggle = function(value)
		...
		if value then
			...
			Pear:CreateAnimation(ConfigWindowFrame,0.3,{
				Size = UDim2.new(0, 185, 0, 195),
			})

			Pear:CreateAnimation(UIStroke,0.3,{
				Transparency = 0
			})

			Pear:CreateAnimation(DropShadow,0.3,{
				ImageTransparency = 0.750
			})

			Pear:CreateAnimation(ScrollingFrame,0.3,{
				BackgroundTransparency = 0
			})

			Pear:CreateAnimation(UIStroke_2,0.3,{
				Transparency = 0.750
			})

			Pear:CreateAnimation(UIStroke_3,0.3,{
				Transparency = 0.750
			})

			Pear:CreateAnimation(SpaceBox,0.3,{
				BackgroundTransparency = 0
			})

			Pear:CreateAnimation(InputFrame,0.3,{
				BackgroundTransparency = 0
			})

			Pear:CreateAnimation(TextBox,0.3,{
				TextTransparency = 0.5
			})

			Pear:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Pear:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Pear:CreateAnimation(SaveConfigButton,0.3,{
				ImageTransparency = 0.500
			})

			Pear:CreateAnimation(DeleteButton,0.3,{
			...
			Pear:CreateAnimation(ConfigWindowFrame,0.275,{
	...
	ScrollingFrame.Active = true
	ScrollingFrame.BackgroundColor3 = Pear.Colors.Black
	ScrollingFrame.BorderColor3 = Color3.fromRGB(0, 0, 0)
	ScrollingFrame.BorderSizePixel = 0
	...
	InputFrame.BackgroundColor3 = Pear.Colors.Black
	...
	TextBox.ZIndex = 209
	TextBox.FontFace = Pear.FontSemiBold
	TextBox.Text = ""
	...
	DeleteButton.ImageColor3 = Pear.Colors.Main
	...
	Pear:CreateHover(SaveConfigButton,function(bool)
		if bool then
			Pear:CreateAnimation(SaveConfigButton,0.3,{
	...
	Pear:CreateHover(GetConfigButton,function(bool)
		if bool then
			Pear:CreateAnimation(GetConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	Pear:CreateHover(AddConfigButton,function(bool)
		if bool then
			Pear:CreateAnimation(AddConfigButton,0.3,{
				ImageTransparency = 0.15;
			})
	...
	res = Pear:CreateResponse({
		...
		Refresh = function()
			...
			for i,v in next, Configs do
				...
				if tostring(import.Name) == tostring(v) or import.Name == v then
					...
					import.Name = v;
					import.Element.TextColor3 = Pear.Colors.Main;
					button.Text = "  "..tostring(v);
				end;
				...
				button.MouseButton1Click:Connect(function()
					...
					import.Element = button;
					import.Name = v;
					button.TextColor3 = Pear.Colors.Main;
					button.Text = "  "..tostring(v);
				end);
			end;
		end,
		...
		Init = function(Name: string,Folder: string)
			if not isfolder(Folder or "Pear") then
				makefolder(Folder or "Pear");	
			end;

			if not isfolder((Folder or "Pear").."/Config") then
				makefolder((Folder or "Pear").."/Config");	
			end;

			local cfgPath = (Folder or "Pear").."/Config/"..tostring(Name);
...
function Pear.new(Window: Window)
	Window = Window or {};
	...
	Pear:ScrollSignal(tbc,UIListLayout,'X');
	...
	FatalFrame:GetPropertyChangedSignal("BackgroundTransparency"):Connect(function()
		if FatalFrame.BackgroundTransparency >= 0.99 then
			Pearwin.Enabled = false;
		else
			Pearwin.Enabled = true;
		end;
	end);
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Pear:CreateAnimation(Header,0.5,{
				BackgroundTransparency = 0,
			})

			Pear:CreateAnimation(HeaderLine,0.5,{
				BackgroundTransparency = 0,
			})

			Pear:CreateAnimation(Bottom,0.5,{
				BackgroundTransparency = 0,
			})

			Pear:CreateAnimation(HeaderLine_2,0.5,{
				BackgroundTransparency = 0,
			})

			Pear:CreateAnimation(HeaderLineShadow,0.5,{
				BackgroundTransparency = 0.5,
			})

			Pear:CreateAnimation(HeaderLineShadow_2,0.5,{
				BackgroundTransparency = 0.5,
			})

			Pear:CreateAnimation(InfoButton,0.25,{
				ImageTransparency = 0.5
			})

			Pear:CreateAnimation(SearchButton,0.25,{
				ImageTransparency = 0.5
			})

			Pear:CreateAnimation(SaveButton,0.25,{
				ImageTransparency = 0.5
			})

			Pear:CreateAnimation(HeaderText,0.35,{
				TextStrokeTransparency = 0.640,
				TextTransparency = 0
			})

			Pear:CreateAnimation(UserIcon,0.45,{
				ImageTransparency = 0,
				Position = UDim2.new(1, -10,0.5, 0)
			})

			Pear:CreateAnimation(User_name,0.35,{
				TextTransparency = 0,
				Position = UDim2.new(1, -40,0, 3)
			})

			Pear:CreateAnimation(version_label,0.5,{
				TextTransparency = 0,
			...
			Pear:CreateAnimation(InfoButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(1, -5,0.5, 0)
			})

			Pear:CreateAnimation(SearchButton,0.1,Enum.EasingStyle.Back,{
				Position = UDim2.new(0,10,0.5, 0)
			})

			Pear:CreateAnimation(SaveButton,0.1,Enum.EasingStyle.Back,{
			...
			Pear:CreateAnimation(SaveButton,0.35,{
				Position = UDim2.new(0,35,1, 20)
			})

			Pear:CreateAnimation(SearchButton,0.35,{
				Position = UDim2.new(0,10,1, 20)
			})

			Pear:CreateAnimation(InfoButton,0.35,{
			...
			Pear:CreateAnimation(UserIcon,0.35,{
				ImageTransparency = 1,
				Position = UDim2.new(1, -10,0.5, 0)
			})
			...
			Pear:CreateAnimation(version_label,0.1,{
				TextTransparency = 1,
				Position = UDim2.new(1, -40,0, 16)
			})
			...
			Pear:CreateAnimation(FatalFrame,0.75,{
				BackgroundTransparency = 1,
			})
	...
	Pearwin.Name = Pear:RandomString();
	Pearwin.Parent = CoreGui;
	Pearwin.ResetOnSpawn = false;
	Pearwin.IgnoreGuiInset = true;
	Pearwin.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	table.insert(Pear.Windows,Pearwin)

	protect_gui(Pearwin);

	FatalFrame.Active = true;
	FatalFrame.Name = Pear:RandomString()
	FatalFrame.Parent = Pearwin
	...
	HeaderLine.Name = Pear:RandomString()
	...
	HeaderText.Name = Pear:RandomString()
	...
	UserProfle.Name = Pear:RandomString()
	...
	UserIcon.Name = Pear:RandomString()
	...
	User_name.Name = Pear:RandomString()
	...
	version_label.Name = Pear:RandomString()
	version_label.Parent = UserProfle
	version_label.AnchorPoint = Vector2.new(1, 0)
	version_label.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
	version_label.BackgroundTransparency = 1.000
	version_label.BorderColor3 = Color3.fromRGB(0, 0, 0)
	version_label.BorderSizePixel = 0
	version_label.Position = UDim2.new(1, -40, 0, 16)
	version_label.Size = UDim2.new(0, 200, 0, 15)
	version_label.ZIndex = 4
	version_label.Font = Enum.Font.GothamMedium
	version_label.Text = string.format("<font transparency=\"0.5\">version:</font> <font color=\"#a6d388\">%s</font>",Window.Version)
	version_label.TextColor3 = Color3.fromRGB(255, 255, 255)
	version_label.TextSize = 12.000
	version_label.TextStrokeTransparency = 0.700
	version_label.TextXAlignment = Enum.TextXAlignment.Right
	version_label.RichText = true;

	HeaderLineShadow.Name = Pear:RandomString()
	...
	MenuFrame.Name = Pear:RandomString()
	...
	HeaderLine_2.Name = Pear:RandomString()
	...
	HeaderLineShadow_2.Name = Pear:RandomString()
	...
	Pear:Drag(FatalFrame,FatalFrame,0.1);
	...
	function Fatal:AddConfig()
		...
		return Pear:CreateConfigWindow(Pearwin,Fatal,ConfigButton);
	end;
	...
	function Fatal:LoadConfig(config)
		...
		for i,v in next , config do
			if i ~= "Info" then
				local Element = Pear.WindowFlags[Pearwin][i];
	...
	function Fatal:AddMenu(Menu : Menu)
		...
		MenuButton.Name = Pear:RandomString()
		...
		Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
		Icon.ZIndex = 5
		Icon.Image = Pear:GetIcon(Menu.Icon);
		Icon.ImageColor3 = Pear.Colors.Main
		Icon.ScaleType = Enum.ScaleType.Crop

		UICorner_2.CornerRadius = UDim.new(1, 0)
		UICorner_2.Parent = Icon

		menu_name.Name = Pear:RandomString()
		...
		local text_size = Pear:GetTextSize(menu_name);
		...
		MenuLiber.Name = Pear:RandomString()
		...
		if not Menu.AutoFill then
			Pear:ScrollSignal(Right,UIListLayout_3,'Y');
			Pear:ScrollSignal(Center,UIListLayout_2,'Y');
			Pear:ScrollSignal(Left,UIListLayout,'Y');
		else
		...
		Fatal.Signal.Event:Connect(function(Bool)
			if Bool then
				...
				Pear:CreateAnimation(UIStroke,0.5,{
					Transparency = 0.950
				})

				Pear:CreateAnimation(Left,0.3,{
					Position = UDim2.new(0.175, 0, 0.5, 0)
				})

				Pear:CreateAnimation(Center,0.4,{
					Position = UDim2.new(0.5, 0, 0.5, 0)
				})

				Pear:CreateAnimation(Right,0.5,{
		...
		function MenuLib:AddPreview(Config: Preview)
			...
			PreviewName.FontFace = Pear.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Pear:CreateAnimation(Preview,0.5,{
						BackgroundTransparency = 0
					})

					Pear:CreateAnimation(PreviewName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
					
					Pear:CreateAnimation(Main,0.5,{
						BackgroundTransparency = 0
					})
					...
					Pear:CreateAnimation(MainBlock,0.5,{
		...
		function MenuLib:AddListBox(Config : Listbox)
			...
			ListBox.Name = Pear:RandomString()
			...
			Pear:AddDragBlacklist(ScrollingFrame);

			Elements.Name = Pear:RandomString()
			...
			SearchBox.Name = Pear:RandomString()
			SearchBox.Parent = Elements
			SearchBox.BackgroundColor3 = Pear.Colors.Black
			...
			TextBox.BackgroundColor3 = Pear.Colors.Black
			TextBox.BackgroundTransparency = 1.000
			TextBox.BorderColor3 = Color3.fromRGB(0, 0, 0)
			TextBox.BorderSizePixel = 0
			...
			ListName.Name = Pear:RandomString()
			...
			ListName.ZIndex = 10
			ListName.FontFace = Pear.FontSemiBold
			...
			local Toggle = function(v)
				if v then
					Pear:CreateAnimation(ListBox,0.5,{
						BackgroundTransparency = 0
					})
					...
					Pear:CreateAnimation(UIStroke_2,0.5,{
						Transparency = 0.75
					})

					Pear:CreateAnimation(UIStroke_3,0.5,{
						Transparency = 0.75
					})

					Pear:CreateAnimation(SearchBox,0.5,{
						BackgroundTransparency = 0
					})

					Pear:CreateAnimation(TextBox,0.5,{
						TextTransparency = 0
					})
					...
					Pear:CreateAnimation(ScrollingFrame,0.45,{
						BackgroundTransparency = 0,
					...
					table.foreach(ScrollingFrame:GetChildren(),function(i,v)
						if v:IsA('TextButton') then
							Pear:CreateAnimation(v,0.45,{
								TextTransparency = 0.5
							});
						end;
					end);

					Pear:CreateAnimation(ListName,0.5,{
				...
				Refresh = function()
					...
					for i,v in next , Config.Values do
						...
						if Config.Multi then
							...
							Pear:CreateHover(bth,function(bool)
								if not Config.Default[v] then
									if bool then
										Pear:CreateAnimation(bth,0.25,{
											TextTransparency = 0;
										})
									else
										Pear:CreateAnimation(bth,0.25,{
			...
			if Config.Flag then
				Pear.WindowFlags[Pearwin][Config.Flag.."ListBox"] = res;
			end;
		...
		function MenuLib:AddSection(Config : Section)
			...
			local Toggle = function(v)
				if v then
					Pear:CreateAnimation(Section,0.5,{
						BackgroundTransparency = 0
					})
					...
					Pear:CreateAnimation(SectionName,0.5,{
						TextStrokeTransparency = 0.750,
						TextTransparency = 0
					})
				else
			...
			Section.Name = Pear:RandomString()
			...
			SectionName.Name = Pear:RandomString()
			...
			SectionName.FontFace = Pear.FontSemiBold;
	...
	do
		InfoButton.Name = Pear:RandomString()
		...
		SaveButton.Name = Pear:RandomString()
		...
		Pear:CreateHover(InfoButton,function(bool)
			if bool then
				Pear:CreateAnimation(InfoButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Pear:CreateAnimation(InfoButton,0.5,{
	...
	do
		...
		Pear:AddDragBlacklist(SearchFrame);
		...
		local SearchToggle = function(value)
			if value then
				...
				Pear:CreateAnimation(SearchFrame,0.35,{
					Size = searchScale
				})
				...
				Pear:CreateAnimation(SearchBox,0.35,{
					BackgroundTransparency = 0
				})
		...
		SearchFrame.Name = Pear:RandomString()
		SearchFrame.Parent = Pearwin;
		...
		Pear:CreateHover(SearchButton,function(bool)
			if bool then
				Pear:CreateAnimation(SearchButton,0.5,{
					ImageTransparency = 0.1
				})
			else
				Pear:CreateAnimation(SearchButton,0.5,{
		...
		local get_button = function(Name,Path,TAB_WARP)
			...
			ResultFrame.Name = Pear:RandomString()
			ResultFrame.Parent = ScrollingFrame;
			ResultFrame.BackgroundColor3 = Pear.Colors.Black
			...
			FeatureName.Name = Pear:RandomString()
			...
			FeatureName.FontFace = Pear.FontSemiBold
			...
			FeaturePath.Name = Pear:RandomString()
			...
			FeaturePath.FontFace = Pear.FontSemiBold
			...
			local button = Pear:NewInput(ResultFrame);

			Pear:CreateHover(button,function(bool)
				if bool then
					Pear:CreateAnimation(ResultFrame,0.5,{
						BackgroundTransparency = 0
					})
				else
					Pear:CreateAnimation(ResultFrame,0.5,{
	...
	function Fatal:GetButton()
		...
		backpack.Name = "PEAR"..Pear:RandomString();
		...
		backpack.MouseEnter:Connect(function()
			Pear:CreateAnimation(backpack,0.3,nil,{
				BackgroundColor3 = Pear.Colors.Main,
...
function Pear:Loader(Config: Loader)
	Config = Config or {};
	Config.Name = Config.Name or "Pear";
	Config.Icon = Config.Icon or "🍐";
	Config.Font = Config.Font or Enum.Font.ArialBold;
	Config.Duration = Config.Duration or 1.5;
	...
	local iconBounds = TextService:GetTextSize(tostring(Config.Icon), textSize, Config.Font, Vector2.new(math.huge, math.huge))
	...
	-- soft "radial" glow behind the intro text (no rectangles / no borders)
	-- this is a stacked-oval falloff + a multi-layer text shadow, so it reads like a glow
	local GlowHolder = Instance.new("Frame")
	GlowHolder.Name = Pear:RandomString()
	GlowHolder.Parent = reveal
	...
	local glowLayers = {}
	local function addGlowLayer(pxGrow: number, alpha: number)
		...
		layer.Size = UDim2.new(1, pxGrow, 1, pxGrow)
		layer.ZIndex = 1
		...
		table.insert(glowLayers, {layer, alpha})
		return layer
	end

	-- stacked ovals = fake radial falloff (stretched across the whole text)
	addGlowLayer(0, 0.86)
	addGlowLayer(8 * Config.Scale, 0.90)
	addGlowLayer(16 * Config.Scale, 0.94)
	addGlowLayer(26 * Config.Scale, 0.97)
	addGlowLayer(38 * Config.Scale, 0.99)
	...
	-- multi-layer text "shadow glow" (black, soft, radial-ish)
	local function offsetUDim2(u: UDim2, dx: number, dy: number)
		return UDim2.new(u.X.Scale, u.X.Offset + dx, u.Y.Scale, u.Y.Offset + dy)
	end

	local function makeTextGlow(lbl: TextLabel)
		local glows = {}

		local function add(dx: number, dy: number, alpha: number, sizeBump: number?, textBump: number?)
			local g = lbl:Clone()
			g.Name = Pear:RandomString()
			g.Parent = lbl.Parent
			g.ZIndex = math.max(lbl.ZIndex - 1, 1)
			...
			g.Position = offsetUDim2(lbl.Position, dx, dy)
			if sizeBump and sizeBump ~= 0 then
				g.Size = offsetUDim2(lbl.Size, sizeBump, sizeBump)
			else
				g.Size = lbl.Size
			end

			if textBump and textBump ~= 0 then
				g.TextSize = lbl.TextSize + textBump
			else
				g.TextSize = lbl.TextSize
			end

			table.insert(glows, {g, alpha})
			return g
		end

		local d1 = math.max(1, math.floor(1 * Config.Scale))
		local d2 = math.max(2, math.floor(2 * Config.Scale))

		-- big faint bloom layers (this is what makes it feel like a soft "cloud")
		add(0, 0, 0.95, 0, math.floor(2 * Config.Scale))
		add(0, 0, 0.98, 0, math.floor(4 * Config.Scale))

		-- inner ring
		local inner = 0.91
		add( d1, 0, inner, 0, 0); add(-d1, 0, inner, 0, 0)
		add( 0, d1, inner, 0, 0); add( 0,-d1, inner, 0, 0)
		add( d1, d1, inner, 0, 0); add(-d1, d1, inner, 0, 0)
		add( d1,-d1, inner, 0, 0); add(-d1,-d1, inner, 0, 0)

		-- outer ring
		local outer = 0.96
		add( d2, 0, outer, 0, 0); add(-d2, 0, outer, 0, 0)
		add( 0, d2, outer, 0, 0); add( 0,-d2, outer, 0, 0)
		add( d2, d2, outer, 0, 0); add(-d2, d2, outer, 0, 0)
		add( d2,-d2, outer, 0, 0); add(-d2,-d2, outer, 0, 0)

		return glows
	end

	local IconGlow = makeTextGlow(IconLabel)
	local NameGlow = makeTextGlow(NameLabel)
	...
	-- glow sizing + fade-in (soft black cloud + radial-ish text glow)
	local glowW = contentWidth + (revealPadding * 6)
	local glowH = math.floor(contentHeight * 1.75)

	Pear:CreateAnimation(GlowHolder,revealTime,nil,{
		Size = UDim2.new(0, glowW, 0, glowH)
	})

	for _,pair in ipairs(glowLayers) do
		Pear:CreateAnimation(pair[1],revealTime,nil,{
			BackgroundTransparency = pair[2]
		})
	end

	for _,pair in ipairs(IconGlow) do
		Pear:CreateAnimation(pair[1],revealTime,nil,{
			TextTransparency = pair[2]
		})
	end

	for _,pair in ipairs(NameGlow) do
		Pear:CreateAnimation(pair[1],revealTime,nil,{
			TextTransparency = pair[2]
		})
	end
	...
	-- fade the glow out with the intro text
	for _,pair in ipairs(IconGlow) do
		Pear:CreateAnimation(pair[1],fadeTime,nil,{
			TextTransparency = 1
		})
	end
...
function Pear:CreateNotifier(): Notifier
	...
	local res = Pear:CreateResponse({
		Notify = function(Config: Notify)
			...
			notify_block.Name = Pear:RandomString()
			...
			Icon.BackgroundColor3 = Pear.Colors.Main
			Icon.BackgroundTransparency = 1.000
			Icon.BorderColor3 = Color3.fromRGB(0, 0, 0)
			Icon.BorderSizePixel = 0
			Icon.Position = UDim2.new(0, 7, 0, 7)
			Icon.Size = UDim2.new(0, 18, 0, 18)
			Icon.Image = Pear:GetIcon(Config.Icon);
			Icon.ImageColor3 = Pear.Colors.Main
			...
			HeaderText.FontFace = Pear.FontSemiBold
			HeaderText.Text = Config.Title
			HeaderText.TextColor3 = Pear.Colors.Main
			HeaderText.TextSize = 13.000
			HeaderText.TextTransparency = 1
			HeaderText.TextXAlignment = Enum.TextXAlignment.Left

			BodyText.Name = Pear:RandomString()
			...
			BodyText.ZIndex = 55
			BodyText.FontFace = Pear.FontSemiBold
			...
			local updateScale = function()
				local TitleScale = Pear:GetTextSize(HeaderText,Enum.Font.GothamBold);
				local ContentScale = Pear:GetTextSize(BodyText,Enum.Font.GothamMedium);
				...
				Pear:CreateAnimation(notify_block,0.35,{
					Position = UDim2.new(0.5, 0, 0, 0)
				});
			end;

			task.delay(0.1,function()
				Pear:CreateAnimation(notify_block,0.25,{
					BackgroundTransparency = 0.1
				})

				Pear:CreateAnimation(UIStroke,0.25,{
					Thickness = 2.500,
					Transparency = 0.900
				})

				Pear:CreateAnimation(Icon,0.25,{
					ImageTransparency = 0
				})

				Pear:CreateAnimation(HeaderText,0.25,{
					TextTransparency = 0.200
				})

				Pear:CreateAnimation(BodyText,0.25,{
			...
			task.delay(Config.Duration + 0.25,function()
				...
				task.delay(0.35,function()
					Pear:CreateAnimation(notify,0.5,{
						Size = UDim2.new(0,0,0,0)
					})
	...
	Pear.__NOTIFIER_CACHE = res;

	return res;
end;

Pear.PEAR_PID = Pear:RandomString();
